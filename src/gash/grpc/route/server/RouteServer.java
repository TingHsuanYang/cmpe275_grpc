...
public class RouteServer {
	...
	private void init() {
		...
		// extract next server information
		 tmp = conf.getProperty("server.next.id");
        if (tmp == null)
            throw new RuntimeException("missing next server ID");
		nextServerID = Long.parseLong(tmp);

        tmp = conf.getProperty("server.next.port");
        if (tmp == null)
            throw new RuntimeException("missing next server port");
        nextServerPort = Integer.parseInt(tmp);
        if (nextServerPort <= 1024)
            throw new RuntimeException("server port must be above 1024");
